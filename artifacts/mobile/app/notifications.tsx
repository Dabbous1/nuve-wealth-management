    ...
    >
      ...
      {notifications.length === 0 ? (
        <View style={styles.empty}>
          <Feather name="bell-off" size={48} color={C.grayLight} />
          <NuveText variant="body" color={C.textMuted}>{s.noNotifications}</NuveText>
        </View>
      ) : (
        notifications.map(notif => (
          <TouchableOpacity
            key={notif.id}
            style={[styles.notifCard, { backgroundColor: C.white, shadowColor: Colors.midnight }, !notif.read && { backgroundColor: C.teal + '08', borderLeftWidth: 3, borderLeftColor: C.teal }]}
            onPress={() => markNotificationRead(notif.id)}
          >
            {!notif.read && <View style={[styles.unreadDot, { backgroundColor: C.teal }]} />}
            ...
            <View style={{ flex: 1 }}>
              ...
              <NuveText variant="caption" color={C.textSecondary} style={{ marginTop: 2, lineHeight: 18 }}>
                {language === 'ar' ? notif.bodyAr : notif.body}
              </NuveText>
              <NuveText variant="caption" color={C.textMuted} style={{ marginTop: 4 }}>
